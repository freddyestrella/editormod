...
{
    public class XG_ListBoxItem
    {
        public object Value;
        public object Tag;

        public XG_ListBoxItem(object value)
        {
            Value = value;
            Tag = null;
        }

        public XG_ListBoxItem(object value, object tag)
        {
            Value = value;
            Tag = tag;
        }
    ...
    public class XGListBoxItemCollection
    {
        ...
        private List<XG_ListBoxItem> Items = new List<XG_ListBoxItem>();

        public XG_ListBoxItem this[int index]
        {
            get 
            { 
                return Items[index]; 
        ...
        {
            Items.Add(new XG_ListBoxItem(value));
            NotifyListChanged();
        }
    ...
    public class XGListBox : XG_Control
    {
        ...
        public XG_ListBoxItem SelectedItem
        {
            get
            {
                if (SelectedIndex >= 0)
        ...
        void DoLayout()
        {
            ...
            if (Items.Count > 0)
            {
                ...
                scrollBar.Value = (float)TopItemIndex;// / (float)Items.Count; // keep scrollbar in place for current name
            }
            else
            {
        ...
        public override void Draw(float frameTime)
        {
            ...
            if (XnaGUIManager.HasFocus(this))
                DrawBorder(guirec, 1, ForeColor, true);
            else
            ...
            */
            ...
            for (int i = TopItemIndex; i < lastItem; i++)
            {
                if (i == SelectedIndex)
                {
                    ...
                    XnaGUIManager.spriteBatch.Draw(XnaGUIManager.WhiteRect, itemRect, ControlDarkColor);
                }
