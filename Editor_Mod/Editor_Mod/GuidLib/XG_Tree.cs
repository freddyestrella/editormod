...
namespace Editor_Mod
{
    ...
    public class XGTree : XG_Panel
    {
        ...
        void Construct()
        {
            ...
            scrollBar = new XGVScrollbar(new Rectangle(Rectangle.Width - 20, 1, 20, Rectangle.Height - 2), this.Scrollbar_ValueChanged);
        ...
        void SelectNext(bool autoExpand)
        {
            ...
            // Select start child of current name if available
            ...
            if (selItem == null)
            {
                // we came out, so we have to go to our next name
        ...
        {
            base.Draw(frameTime); // Let XG_Panel draw the borders and background
    ...
    public class XGTreeItem
    {
        ...
        /// <summary>
        /// Called by XGTree.Update when Active is down in it's control area
        /// (Will call child items Update if this name is Expanded)
        /// </summary>
        /// <param name="frameTime"></param>
        /// <param name="mousePoint"></param>
        /// <param name="guirec">Rectangle area for TreeItem, gets modified during propagation.</param>
        ...
        /// <summary>
        /// Get the Nth displayable name in the list 
        ...
        internal bool Draw(float frameTime, ref Rectangle rect, XGTreeDrawingContext context)
        {
            ...
            if (context.DoDraw)
            {
                ...
                if (haveChildren)
                {
                    XG_Control.DrawBorder(expRect, 1, Root.ThisControlColor, true);
                    ...
                    r.Y += r.Height / 2;
                    r.Height = 1;
                    XnaGUIManager.spriteBatch.Draw(XnaGUIManager.WhiteRect, r, Root.ThisControlColor);
                    if (!drawChildren)
                    {
                        r = pmRect;
                        r.X += r.Width / 2;
                ...
                {
                    XnaGUIManager.spriteBatch.Draw(XnaGUIManager.WhiteRect, textRect, XG_Control.ControlDarkColor);
                }

                Vector2 textPos = XG_Control.GetTextPosition(textRect, Value.ToString(), GUIAlignment.Left | GUIAlignment.VCenter, Root.Padding);
                XnaGUIManager.spriteBatch.DrawString(XnaGUIManager.spriteFont, Value.ToString(), textPos, Root.ThisForeColor);

                rect.Y += Root.ItemHeight; // step down to next name
            }
