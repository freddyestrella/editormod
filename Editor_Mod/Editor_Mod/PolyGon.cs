...
namespace Editor_Mod
{
    public class PolyGon
    {
        ...
        {
            return new Vector2(((this.x1 + this.x2) / 2), this.y1);
        }
        public Vector2 CenterBotton()
        {
            return new Vector2(((this.x1 + this.x2) / 2), this.y2);
        }
        public Vector2 CenterRight()
        {
            return new Vector2(this.x2, ((this.y1 + this.y2) / 2));
        }
        public Vector2 CenterLeft()
        {
            return new Vector2(this.x1, ((this.y1 + this.y2) / 2));
        }
        ...
        {
            return new Vector2(this.x2, this.y1);
        }
        public Vector2 CornerBottonLeft()
        {
            return new Vector2(this.x1, this.y2);
        }
        ...
        public void getDimentions()
        {
            ...
            {
                if (this.verTices[i].X < this.x1 || this.x1 == 0)
                {
                    this.x1 = (int)this.verTices[i].X;

                }
                if (this.verTices[i].Y < this.y1 || this.y1 == 0)
                {
                    this.y1 = (int)this.verTices[i].Y;

                }
                if (this.verTices[i].X > this.x2 || this.x2 == 0)
                {
                    this.x2 = (int)this.verTices[i].X;

                }
                if (this.verTices[i].Y > this.y2 || this.y2 == 0)
                {
                    this.y2 = (int)this.verTices[i].Y;
                }
            }
        }
        ...
        private Vector2 RotatePointM(Vector2 point, Vector2 origin, float rotation)
        {
            Matrix m =
        ...
        private List<Vector2> CreateArc(Vector2 center = new Vector2(),double radius = 0, int sides = 0, float startingAngle =0, float degrees =0)
        {
            ...
            if (radius == 0) { this.getDimentions(); radius = Vector2.Distance(new Vector2(this.x1, this.y1), new Vector2(this.x2, this.y1)) / 2; }
        ...
        public void CreateTriangle(Vector2 center = new Vector2(), float radius = 0)
        {
            ...
            if (radius == 0) { radius = Vector2.Distance(new Vector2(this.x1, this.y1), new Vector2(this.x2, this.y1))/2; }
            List<Vector2> vectors = new List<Vector2>();
               ...
               // this.activeFill.Clear();
                ...
                for (int i = 0; i < this.verTices.Count; i++)
                {
                    if (i > 0)
                    {
                        ...
                        traceLine2(this.verTices[i - 1], this.verTices[i]);
        ...
        public void traceLine1(Vector2 point1, Vector2 point2)
        {
        ...
        public void traceLine2(Vector2 point1, Vector2 point2)
        {
            float distance = Vector2.Distance(point1, point2);
            Vector2 direction = ((point2 - point1) / distance);
            direction.Normalize();
