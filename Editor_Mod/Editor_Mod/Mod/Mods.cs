...
namespace Editor_Mod
{
    ...
    {
        public static string  Editorversion = @"2.0 - beta 4";
        ...
        public static bool WallMode;
        public static Vector2 Mouse;
        public float playersX { get { return Main.screenWidth - PlayersX; } set { PlayersX = value; } }
        public static bool canTeleport { get; set; }
        public static bool leftNoClicked { get; set; }
 
        public static Color FontColor = Color.White;
        public static Color ControlColor = Color.Gray;
        public static Color BkgColor = Color.Black;
        public static float ForeColorValue = 0.600f;
        public static float ControlColorValue = 0.4f;
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            Mouse = new Vector2(((float)Math.Truncate((float)Main.mouseState.X + Main.screenPosition.X) / 16f), (float)Math.Truncate(((float)Main.mouseState.Y + Main.screenPosition.Y) / 16f));
            if (Main.gameMenu)
            {
                return;
            }
      ...
      if ((keyboardState.IsKeyDown(Keys.LeftControl) || keyboardState.IsKeyDown(Keys.RightControl)) && keyboardState.IsKeyDown(Keys.C))
            {
                ...
                keyPressed = Keys.C;
                if (Mod.toolWindow.brush.copyPaste != null)
                {
            ...
            else if ((keyboardState.IsKeyDown(Keys.LeftControl) || keyboardState.IsKeyDown(Keys.RightControl)) && keyboardState.IsKeyDown(Keys.V))
            {
                ...
                keyPressed = Keys.V; if (Mod.toolWindow.brush.copyPaste != null)
                {
                    if (Mod.toolWindow.brush.copyPaste != null)
                    {
      ...
      else if ((keyboardState.IsKeyDown(Keys.LeftShift) || keyboardState.IsKeyDown(Keys.RightShift)) && keyboardState.IsKeyDown(Keys.Z))
      {
        ...
        {
            ThreadPool.QueueUserWorkItem(new WaitCallback(pasting), 1);
        }
        public static void pasting(object threadContext)
        {
            Pastethis();
        }
        public static void startcopying()
        {
            ThreadPool.QueueUserWorkItem(new WaitCallback(copying), 1);
        }
        public static void copying(object threadContext)
        {
            Copythis();
        }
            ...
            if (count.Count > 0)
            {
                ...
                if (Main.netMode == 1)
                {
                    for (int i = 0; i < count.Count; i++)
                    {
                        ...
                        if (Main.tile[(int)count[i].loc.X, (int)count[i].loc.Y].liquid > 0)
                        {
        ...
        private void updateWindows(GameTime gameTime)
        {
            ...
            if (Main.mouseState.LeftButton == ButtonState.Pressed)
            {
                if (Mod.leftNoClicked)
                {
                    ...
                    Mod.leftNoClicked = false;
                }
            }
            else { Mod.leftNoClicked = true; }
        ...
        {
            if (Mod.toolWindow.playerS.player_CH.Light.Value)
            {
            ...
            if (toolWindow.playerS.player_CH.Fly.Value ) { Mod.UpdateNoclip(); }
 
            if (toolWindow.playerS.player_CH.infAir.Value)
            {
                Main.player[Main.myPlayer].breath = Main.player[Main.myPlayer].breathMax;
            }
            if (toolWindow.playerS.player_CH.inflife.Value)
            {
            ...
            if (toolWindow.playerS.player_CH.infmana.Value)
            {
                Main.player[Main.myPlayer].statMana = Main.player[Main.myPlayer].statManaMax = Main.player[Main.myPlayer].statManaMax2 = 200;
            }

    
            if (toolWindow.playerS.player_CH.blockRange.Value > 0)
            {

                Main.player[Main.myPlayer].inventory[Main.player[Main.myPlayer].selectedItem].tileBoost = (int)toolWindow.playerS.player_CH.blockRange.Value;

            }


            toolWindow.playerS.player_CH.itemGrabRange.DisplayValue = PlayerReflect.itemGrabRange;
            toolWindow.playerS.player_CH.itemGrabSpeed.DisplayValue = PlayerReflect.itemGrabSpeed;
            ...
            if (toolWindow.playerS.player_CH.rocketTimeMax.Value > 0)
            {
                Main.player[Main.myPlayer].rocketTimeMax =(int) toolWindow.playerS.player_CH.rocketTimeMax.Value;
            }
        ...
        public static string addquotes(string[] text, bool mergeonly = false)
        {
            bool add = false;
            string result = "";
  ...
  if (Mod.toolWindow.brush.copyPaste != null)
  {
      ...
      if (Mod.toolWindow.brush.copyPaste.Visible)
      {
        ...
        #endregion
        #region placement
        public static void placecustom(int i, int j, int type, int style)
        {
            ...
            if (type == 80)
            {
                ...
                WorldGenReflect.GrowCactus(i, j);
                //  SquareTileFrame(j, X, true);
            ...
            if (type == 72)
            {
                ...
                WorldGenReflect.GrowShroom(i, j);
            }

            if (type == 28)
            {
                WorldGenReflect.PlacePot(i, j, 28);
                WorldGenReflect.SquareTileFrame(i, j, true);
            }
            ...
            if (type == 20)
            {
                ...
                {
                    WorldGenReflect.Place1x2(i, j, type, style);
                    WorldGenReflect.SquareTileFrame(i, j, true);

                    WorldGenReflect.GrowEpicTree(i, j);
                    WorldGenReflect.SquareTileFrame(i, j, true);


                    WorldGenReflect.GrowTree(i, j);
                    WorldGenReflect.SquareTileFrame(i, j, true);
            ...
            if (type == 12)
            {
                ...
                WorldGenReflect.AddLifeCrystal(i, j);
                WorldGenReflect.SquareTileFrame(i, j);
                mySendTileSquare(-1, i, j, 4);
            }
        }
        public static void mySquareTileFrame(int i, int j, bool resetFrame = false, bool nobreak = true)
        {
            WorldGenReflect.TileFrame(i - 1, j - 1, resetFrame, nobreak);
            WorldGenReflect.TileFrame(i - 1, j, resetFrame, nobreak);
            WorldGenReflect.TileFrame(i - 1, j + 1, resetFrame, nobreak);
            WorldGenReflect.TileFrame(i, j - 1, resetFrame, nobreak);
            WorldGenReflect.TileFrame(i, j, resetFrame, nobreak);
            WorldGenReflect.TileFrame(i, j + 1, resetFrame, nobreak);
            WorldGenReflect.TileFrame(i + 1, j - 1, resetFrame, nobreak);
            WorldGenReflect.TileFrame(i + 1, j, resetFrame, nobreak);
            WorldGenReflect.TileFrame(i + 1, j + 1, resetFrame, nobreak);
        }
        public static void mySendTileSquare(int whoAmi, int tileX, int tileY, int size)
        {
            int num = (size - 1) / 2;
            SendPackets.SendData(120, whoAmi, -1, "", size, (float)(tileX - num), (float)(tileY - num), 0f, 0);
        }
        public static bool AddLifeCrystal(int i, int j)
        {

            if (!Main.tile[i, j].active && Main.tileSolid[Main.tile[i, j + 1].type])
            {
                int endX = i;
                int endY = j;
                ...
                if (!WorldGenReflect.EmptyTileCheck(endX - 1, endX, endY - 1, endY, -1))
                {
                    return false;
                }
        ...
        public static bool placetilesandwalls(bool tile, int X, int Y, int type, int PlaceStyle, bool force = false, bool send = true)
        {
            ...
            if (X < 0) { X = 0; }
            if (Y < 0) { Y = 0; }
            if (X > Main.maxTilesX) { X = Main.maxTilesX; }
            if (Y > Main.maxTilesY) { Y = Main.maxTilesY; }
            ...
            if (tile)
            {
                if (thisTILE >= 0)
                {
                    if (((!Main.tile[X, Y].active && (!Main.tile[X, Y].lava)) || (((thisTILE == 23) || (thisTILE == 2)) || ((thisTILE == 60) || (thisTILE == 70) || (thisTILE == 80)))))
                    {
                        //if (thisTILE == 12 && (!Main.tile[X, Y + 1].active || !Main.tileSolid[Main.tile[X, Y + 1].type] || (Main.tile[X - 1, Y].active) || (Main.tile[X - 2, Y].active)))
                        //{
                        //    return;
                        //}
                        if (thisTILE == 12)
                        {
                            ...
                            if (AddLifeCrystal(X, Y))
                            {
                                if (send)
                                {
                                    SendPackets.SendTileSquare(-1, X, Y, 4);
                                }
                        ...
                        if (thisTILE == 5 && (Main.tile[X, Y + 1].type == 5 && Main.tile[X, Y + 1].active) || Main.tile[X, Y].type == 5)
                        {

                            goto continueit;
                        }
                        if (thisTILE == 5 && (Main.tile[X, Y + 1].type != 2 || !Main.tile[X, Y + 1].active))
                        {
                        ...
                        if (thisTILE == 72 && (Main.tile[X, Y + 1].type != 70 || !Main.tile[X, Y + 1].active))
                        {

                            return false;
                        }
                        else if (thisTILE == 72 && WorldGenReflect.EmptyTileCheck(X - 2, X + 2, Y - 13, Y - 1, 71))
                        {
                            WorldGenReflect.PlaceTile(X, Y, 70, false, false);
                            SendPackets.SendData(217, -1, -1, "", 1, (float)X, (float)Y, (float)70, -1);
                        ...
                        if (thisTILE == 80 && (!Main.tile[X, Y].active && ((Main.tile[X - 1, Y].type == 80 && Main.tile[X - 1, Y].frameX == 0) || (Main.tile[X + 1, Y].type == 80 && Main.tile[X + 1, Y].frameX == 0)) && (Main.tile[X - 1, Y - 1].type != 5 || Main.tile[X + 1, Y - 1].type != 5 || Main.tile[X - 1, Y + 1].type != 5 || Main.tile[X + 1, Y + 1].type != 5 || Main.tile[X, Y].type != 5)))
                        {
                        ...
                        if (thisTILE == 80 && ((!Main.tile[X, Y].active) && ((Main.tile[X, Y + 1].active && Main.tile[X, Y + 1].type == 80) || (Main.tile[X, Y + 1].active && Main.tile[X, Y + 1].type == 53))))
                        {
                            goto continueit;
                        }
                    ...
                    continueit:
                        ...
                        // if ((thisTILE == 107) || (thisTILE == 108) || (thisTILE == 109) || (thisTILE == 70) || (thisTILE == 80) || (thisTILE == 72) || (thisTILE == 28) || (thisTILE == 20) || (thisTILE == 12))
                        //   {
                        //      placecustom(X, Y, thisTILE, PlaceStyle);
                        //    return false;

                        //  }


                        if (WorldGenReflect.PlaceTile(X, Y, thisTILE, false, force, ME, PlaceStyle))
                        {
                            if (send)
                            {

                                if (thisTILE == 2 || thisTILE == 23)
                                {


                                    //  WorldGen.PlaceTile(X, Y, 0, false, false);
                                    SendPackets.SendData(117, -1, -1, "", 1, (float)X, (float)Y, (float)0, 0);

                                }
                                if (thisTILE == 60)
                                {

                                    //  WorldGen.PlaceTile(X, Y, 59, false, false);
                                    SendPackets.SendData(117, -1, -1, "", 1, (float)X, (float)Y, (float)59, 0);
                                ...
                                if (Main.netMode == 1)
                                {
                                    SendPackets.SendData(117, -1, -1, "", 1, (float)X, (float)Y, (float)thisTILE, PlaceStyle);
                                }
                                if (thisTILE == 15)
                                {
                                    if (Main.player[myPlayer].direction == 1)
                                    {
                                ...
                                else if ((thisTILE == 79) && (Main.netMode == 1))
                                {
            ...
            else
            {
                ...
                if (thisWALL >= 0)
                {
                    WorldGenReflect.PlaceWall(X, Y, thisWALL, false);
                    if (send)
                    {
                        if (Main.netMode == 1)
                        {
                            SendPackets.SendData(117, -1, -1, "", 3, (float)X, (float)Y, (float)thisWALL, 0);
                        }
                    }
