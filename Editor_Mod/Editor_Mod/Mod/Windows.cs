...
namespace Editor_Mod
{
    ...
    public class ToolWindow : XG_Control
    {
        ...
        public Mod Game;
        public XGTabControl MainTabControl { get; protected set; }
            ...
            : base(new Rectangle(X, Y, 420, 180), true)
        {
            ...
            MainTabControl = new XGTabControl(pageRect);
            Children.Add(MainTabControl); // add the tab control to our child control list

            modify = new Modify(game, pageRect);
            MainTabControl.Children.Add(modify);
 
            brush = new Brush(game, pageRect);
            MainTabControl.Children.Add(brush);

            npcs = new Npcs(game, pageRect);
            MainTabControl.Children.Add(npcs);

            itemss = new ItemsS(game, pageRect);
            MainTabControl.Children.Add(itemss);
 

            playerS = new PlayerS(game, pageRect);
            MainTabControl.Children.Add(playerS);

            lazyInv = new InV(game, pageRect);
            MainTabControl.Children.Add(lazyInv);
            server = new Server(game, pageRect);
            MainTabControl.Children.Add(server);
        }


 
        public InV lazyInv { get; set; }
...
public XG_ButtonLabelBool Fill { get; set; }
    ...
    public class ItemsS : XG_TabPage
    {
        public Mod Game;
        public int sideCounter = 51;
        ...
        public ItemsS(Mod game, Rectangle rect)
            : base(rect, "Items")
        {
            Game = game;
            Rectangle pageRect = this.Rectangle; // tab pages are the size of the tab control
    ...
    public class PlayerS : XG_TabPage
    {
        public Mod Game;
 
   

        public PlayerS(Mod game, Rectangle rect)
            : base(rect, "Player")
        {
            Game = game;
            Rectangle pageRect = this.Rectangle; // tab pages are the size of the tab control
    ...
    public class ServerCommand4 : XG_TabPage
    {
        ...
        private bool getregionname;
        private bool findregionname;
        private int points;
            ...
            : base(rect, "SC4")
        {
            ...
            Children.Add(new XG_Button(new Rectangle(10, 5, 60, 20), "set 1", this.set1_Clicked));
            Children.Add(new XG_Button(new Rectangle(100, 5, 60, 20), "set 2", this.set2_Clicked));

            Children.Add(new XG_Button(new Rectangle(10, 35, 60, 20), "define", this.gen_Clicked));
            Children.Add(new XG_Button(new Rectangle(100, 35, 60, 20), "delete", this.gen_Clicked));
            Children.Add(new XG_Button(new Rectangle(200, 5, 60, 20), "allow", this.allow_Clicked));
           

            Children.Add(new XG_Button(new Rectangle(10, 60, 100, 20), "get region", this.getname_Clicked));
            RegionName = new XGTextBox(new Rectangle(10, 90, 150, 20));
            RegionName.Text = "Region name";
            Children.Add(RegionName);

            Children.Add(new XG_Button(new Rectangle(200, 60, 100, 20), "get player info", this.getnamefromp_Clicked));
            player = new XGTextBox(new Rectangle(200, 90, 150, 20));
            player.Text = "player name";
            Children.Add(player);
            Children.Add(new XG_Button(new Rectangle(350, 90, 60, 20), "list", this.list_Clicked));


            Children.Add(new XG_Button(new Rectangle(290, 15, 60, 20), "up", this.expand_Clicked));

            Children.Add(new XG_Button(new Rectangle(290, 40, 60, 20), "down", this.expand_Clicked));

            Children.Add(new XG_Button(new Rectangle(230, 25, 60, 20), "left", this.expand_Clicked));

            Children.Add(new XG_Button(new Rectangle(350, 25, 60, 20), "right", this.expand_Clicked));




        }
        void expand_Clicked(XG_Control sender)
        {
            NetMessage.SendData(25, -1, -1, @"/region resize " + RegionName.Text +" "+ sender.Text +" 1");
           
        }

        void list_Clicked(XG_Control sender)
        {
            NetMessage.SendData(25, -1, -1, @"/region " + sender.Text);
            findregionname = false;
        }
        void allow_Clicked(XG_Control sender)
        {   
            NetMessage.SendData(25, -1, -1, @"/region allow "+  player.Text + " " + RegionName.Text);
            findregionname = false;
        }
        void gen_Clicked(XG_Control sender)
        {
            NetMessage.SendData(25, -1, -1, @"/region " + sender.Text + " " + RegionName.Text);
            findregionname = false;
        }
        void getname_Clicked(XG_Control sender)
        {
        ...
        void getnamefromp_Clicked(XG_Control sender)
        {
            NetMessage.SendData(25, -1, -1, @"/userinfo " + Mod.SelectedPlayer.name);
         
            findusername = true;
            findregionname = false;
        }
        void set1_Clicked(XG_Control sender)
        {
            NetMessage.SendData(25, -1, -1, @"/region set 1");
            points = 1;
            findregionname = false;
        }
        void set2_Clicked(XG_Control sender)
        {
            NetMessage.SendData(25, -1, -1, @"/region set 2");
            points = 2;
            findregionname = false;
        }
        ...
        public override void Update(GameTime gameTime)
        {
                    ...
                    if (Mod.leftNoClicked)
                    {
                        ...
                        if (Main.netMode == 1)
                        {
                            ...
                            if (getregionname)
                            {
                                ...
                                NetMessage.SendData(17, -1, -1, "", 0, (float)Mod.Mouse.X, (float)Mod.Mouse.Y, 0f, 0);
                                findregionname = true;
                                getregionname = false;
                            }
            ...
            if (findregionname || findusername)
            {
                    ...
                    if (words.Length >= 3)
                    {
                        if (words[0] == "Region")
                        {
                            ...
                            if (words[1] == @"Name(s):")
                            {
                                ...
                                for (int e = 2; e < words.Length; e++)
                                {
                                    Arguments[e - 2] = words[e];
                                }
                                RegionName.Text = Mod.addquotes(Arguments);
                                    ...
                                    if (words[3] == "Logged")
                                    {
                                        ...
                                        if (words[4] == "In")
                                        {
                                            if (words[5] == "As:")
                                            {
                                                if (words[6] != "")
                                                {
                                                    string[] Arguments = new string[words.Length - 6];
                                                    for (int e = 6; e < words.Length; e++)
                                                    {
                                                        Arguments[e - 6] = words[e];
                                                    }
                                                    player.Text = Mod.addquotes(Arguments);
                                                }
                                                else
                                                {
                                                    player.Text = Mod.addquotes(Mod.SelectedPlayer.name);
                                                }
                                                findusername = false;
                                            }
                                        }
        ...
        public XGTextBox RegionName { get; set; }

        public XGTextBox player { get; set; }
    }
    ...
    public class Give : XG_TabPage
    {
        ...
        public void NewItem(int X, int Y, string Name = "", int Stack = 1, bool noBroadcast = false)
        {
            ...
            {
                NetMessage.SendData(121, -1, -1, "", index, 0f, 0f, 0f, 0);
